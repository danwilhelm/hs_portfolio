...
void CPan::Pan(void)
{
	...
	/*if(x + destwidth >= src->Width())
	{
		...
		dest->Lock();
		dest->Rect(src2.left, src2.top, src2.right, src2.bottom, xmRGB(255, 0, 0));
		dest->Rect(src1.left, src1.top, src1.right, src1.bottom, xmRGB(0, 255, 0));
		dest->Unlock();
...
void CPan::Create(CSurface *surf, CSurface *bitmap, LPRECT area, int speed, DWORD style)
{
	src  = bitmap;
	dest = surf;
