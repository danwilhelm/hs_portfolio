...
int Init(void)
{
	...
	for(int i=0;i<MAX_STARS;i++)
	{
		...
		star[i].down	= rand()%2;
		star[i].color	= xmRGB(star[i].speed*85, star[i].speed*85, star[i].speed*85);
	}

	for(i=0;i<MAX_POLYS;i++)
	{
		poly[i].Create(vertices, MAX_VERT, xmRGB(rand()%255, rand()%255, rand()%255));
...
int Main(void)
{
	...
	for(i=0;i<yres;i++)
		secondary.PutPixel(i, yres-i, xmRGB(255-chg, 255-chg, 255-chg));
	for(i=yres;i>0;i--)
		secondary.PutPixel(yres-i, yres-i, xmRGB(255-chg, 255-chg, 255-chg));
	for(i=0;i<xres;i++)
		secondary.PutPixel(i, yres>>1, xmRGB(0, chg, 0));
	for(i=0;i<yres;i++)
		secondary.PutPixel(xres>>1, i, xmRGB(0, 0, chg));

	secondary.Line(0, 0, xres >> 1, 255 - chg, xmRGB(128, 255, 128));
	secondary.Line(xres, 0, xres >> 1, chg, xmRGB(255, 128, 255));
	secondary.Line(0, yres, xres >> 1, yres - (255-chg), xmRGB(128, 255, 255));
	secondary.Line(xres, yres, xres >> 1, yres - chg, xmRGB(255, 255, 128));

	circx = cos(circdir) * circrad;
	circy = sin(circdir) * circrad;
	secondary.Line(xres >> 1, yres >> 1, circx + (xres >> 1), circy + (yres >> 1), xmRGB(255, 0, 0));
