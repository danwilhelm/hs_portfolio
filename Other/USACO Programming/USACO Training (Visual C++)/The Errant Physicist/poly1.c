...
TERM polya[200], polyb[200], product[200];
FILE *filein, *fileout;
...
void WritePoly(TERM *terms, long numterms)
{
    ...
    memset(expout, STRSP, 250);
    memset(polyout, STRSP, 250);
    	...
    	if(term->exps[0] != 0)  {
            ...
            if(term->exps[1] >= 1)  {
                ...
                if(term->exps[1] > 1)  {
					...
					if(i != numterms - 1 || term->exps[2] >= 1)  {
						polypos += NumDigits(term->exps[1]);
						expout[polypos] = ' ';
	 					endexp = polypos;
					}
					else
						endexp = polypos + NumDigits(term->exps[1]);
            ...
            if(term->exps[2] >= 1)  {
                ...
                if(term->exps[2] > 1)  {
					...
					if(i != numterms - 1)  {
						...
						endexp = polypos + NumDigits(term->exps[2]);
                }
            }
    	}
    }
...
void SimplifyPoly(TERM *terms, long numterms)
{
    ...
    for(i=0;i<numterms;i++)  {
   	...
   	//sort the terms
    qsort(terms, numterms, sizeof(TERM), (int(*)(const void*,const void*))QSortTerm);
}
